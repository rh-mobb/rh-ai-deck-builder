<!--
  ...
  To adapt for a different domain:
    1. Replace the PHASES array with your own phase snapshots.
    2. Add/remove PodType values and matching CSS classes.
    3. Adjust PHASE_MS for your desired dwell time.
...
function selectPhase(i: number) {
  if (paused.value && i === phaseIndex.value) {
    // clicking the active dot while paused → resume
    paused.value = false
    progress.value = 0
    startTimers()
  } else {
    // jump to phase and pause
    phaseIndex.value = i
    paused.value = true
    progress.value = 100
    stopTimers()
  ...
  <div class="bp-root">
    ...
    <div class="bp-header">
      ...
      <div class="bp-label-sub">
        ...
        <span v-if="paused" class="bp-resume-hint"> · click dot to resume</span>
      </div>
    </div>
...
.bp-resume-hint { color: #555; font-style: italic; }
